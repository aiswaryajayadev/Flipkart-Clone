...
function fetchAccountGender() {
    ...
    if (user) {
        ...
        const userProfileRef = database.ref('UserProfileList/' + userId);
        ...
        userDetailsRef.once('value', (snapshot) => {
            ...
            if (userDetails) {
                ...
                if (formElement) {
                    ...
                    fetchAccountGender(); // Fetch and update gender
                } else {
...
function postAccountName(){
    ...
    if (user) {
        ...
        .then(() => {
            console.log("User's first name, last name, and gender updated successfully!");
        })
        .catch((error) => {
